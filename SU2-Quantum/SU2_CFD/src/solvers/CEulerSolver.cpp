...
void CEulerSolver::ImplicitEuler_Iteration(CGeometry *geometry, CSolver **solver_container, CConfig *config) {
  ...
  DenseVector<su2double> x_my_solution;

  DenseVector<su2double> residual = CSysVector2DenseVec<su2double>(LinSysRes, geometry, nVar, config);
  su2double norm_residual = residual.norm2();

  /*--- Solve or smooth the linear system. ---*/
  if (config->GetUseMyLinearSolver()) {
      iter = 0;
      // DenseVector<su2double> residual = CSysVector2DenseVec(LinSysRes, geometry, nVar, config);
      DenseMatrix<su2double> jacobian = CSysMatrix2DenseMat(Jacobian, geometry, nVar, config);
      bool success = false;
      try {
          x_my_solution = my_linear_solver(jacobian, residual);
          LinSysSol = DenseVec2CSysVector(x_my_solution, nVar);
      }
      catch (runtime_error& err) {
          cout << "Bad Solver, Use Default Solver instead." << endl;
          cout << "ErrMsg: " << err.what() << endl;
          cout << "Now writing the equation to the disk." << endl;
          ...
          residual.write_matlab_file(filenameb);
          print_CSysMatrix_Sparse(&Jacobian, filenameA);

          cout << "The equation has been output to folder /extra. Continue." << endl;

          iter = System.Solve(Jacobian, LinSysRes, LinSysSol, geometry, config);
          x_my_solution = CSysVector2DenseVec<su2double>(LinSysSol, geometry, nVar, config);
      }
  }
  else {
      iter = System.Solve(Jacobian, LinSysRes, LinSysSol, geometry, config);
  ...
  if (config->GetCheckSolverResult()) {
      DenseVector<su2double> residual = CSysVector2DenseVec(LinSysRes, geometry, nVar, config);
      DenseMatrix<su2double> jacobian = CSysMatrix2DenseMat(Jacobian, geometry, nVar, config);
      DenseVector<su2double> test_residual = jacobian * x_my_solution;
      try {
          check_vec<su2double>(test_residual, residual, config->GetCheckSolverResultError());
      }
      catch (runtime_error& err) {
          ...
          string filenameb = "extra\\b." + buffer;
          string filenameA = "extra\\A." + buffer;
          string filenamex = "extra\\x." + buffer;
          residual.write_matlab_file(filenameb);
          x_my_solution.write_matlab_file(filenamex);
          print_CSysMatrix_Sparse(&Jacobian, filenameA);
  ...
  if (config->GetUseComputeUnderRelaxationFactor()) {
      ComputeUnderRelaxationFactor(solver_container, config);
  }

  su2double *Norm2Du = new su2double[nVar];
  ...
  for (int i = 0; i < nVar; ++i) {
      Norm2Du[i] = x_my_solution.norm2(i, nVar);
  }   

  su2double l2norm = x_my_solution.norm2();
  DenseVector<su2double> normalized_x = x_my_solution / l2norm;
  auto extiter = config->GetInnerIter();

  if (config->GetUseQuantum()) {
      // 2. set precision epsilon
      su2double epsilon = config->GetQuantumErrorThreshold();
      // 3. get_normalized

      // 4. pick away all number under the threshold
      int counter = 0;
      su2double sampling_constant = config->GetLinfSamplingConstant();
      ...
      if (config->GetOutputHistogram() && ( extiter % config->GetOutputHistogramFreq() == 0 )) {
          ...
          string buffer = ss.str();
          mkdirectory("extra");
          string filename = "extra\\Histogram." + buffer;
          size_t size = normalized_x.size;
          ofstream out(filename);
          out << "SIZE=" << size << endl;
          out << "SPACE=" << space << endl;
          out << "SMALL={ ";
          for (auto k : small_histo) {
              out << k * 1.0 / size << " ";
          }
          out << "}" << endl;
          out << "BIG={ ";
          for (auto k : big_histo) {
              out << k * 1.0 / size << " ";
          }
          out << "}" << endl;
          out << "OTHER=" << other * 1.0 / size;
      }

      switch (config->GetLinfTomographyVersion()) {
      case 1:
          picked_x = l_inf_tomography(normalized_x, epsilon, sampling_constant, counter);
          break;
      case 2:
          picked_x = l_inf_tomography_v2(normalized_x, epsilon, sampling_constant, counter);
          break;
      case 3:
          picked_x = l_inf_tomography_v3(normalized_x, epsilon, sampling_constant, counter);
          break;
      default:
          picked_x = normalized_x;
          break;
      }
      DenseVector<su2double> x_linf_tomography = picked_x * l2norm;
      // 7. set the LinSysSol
      LinSysSol = DenseVec2CSysVector(x_linf_tomography, nVar);
  }
  else {
      if (config->GetOutputHistogram() && (extiter % config->GetOutputHistogramFreq() == 0)) {
          size_t space = (size_t)config->GetHistogramSpace();
          ...
          for (int i = 0; i < counter.size() - 1;++i) {
              out << "[-" << i << ", -" << i + 1 << "]" << ":\t" << counter[i] << ";\t" << counter[i] * 1.0 / size << endl;
          }
          out << "<-" << counter.size() - 1 << ":\t" << counter[counter.size() - 1] << ";\t" << counter[counter.size() - 1] * 1.0 / size;
      }
  }

  /*--- Write Solution (AgonyTest, Get variation of rho, u, v, (w), E ---*/

  /*  for (unsigned short iVar = 0; iVar < nVar; iVar++) {
        du_norm2[iVar] = Norm2Du[iVar] / norm_residual;
    }*/
  ...
  if (!adjoint) {
    ...
    for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
      for (unsigned short iVar = 0; iVar < nVar; iVar++) {
        su2double factor = nodes->GetUnderRelaxation(iPoint);
		// factor = 1.0;
        nodes->AddSolution(iPoint, iVar, factor*LinSysSol[iPoint*nVar+iVar]);
      }
    }
  }
