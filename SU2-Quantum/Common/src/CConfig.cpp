...
void CConfig::SetConfig_Options() {
  ...
  *  Options: NO, YES \ingroup Config */
  ...
  addDoubleOption("QUANTUM_ERROR_THRESHOLD", quantum_error_threshold, 1e-6);
  addDoubleOption("LINF_SAMPLING_CONSTANT", linf_sampling_constant, 32);
  addBoolOption("USE_QUANTUM", use_quantum, false);
  addBoolOption("USE_COMPUTE_UNDER_RELAXATION_FACTOR", useComputeUnderRelaxationFactor, false);
  addUnsignedShortOption("LINF_TOMOGRAPHY_VERSION", linf_tomography_version, 1);

  addBoolOption("USE_MY_LINEAR_SOLVER", use_my_linear_solver, false);
  addBoolOption("CHECK_SOLVER_RESULT", check_solver_result, false);
  addDoubleOption("CHECK_SOLVER_RESULT_ERROR", check_solver_result_error, false);

  addBoolOption("OUTPUT_HISTOGRAM", output_histogram, false);
  addUnsignedLongOption("HISTOGRAM_SPACE", histogram_space, 10);
  addUnsignedLongOption("OUTPUT_HISTOGRAM_FREQ", output_histogram_freq, 100);
