...
private:
  ...
  *HistoryOutput, *VolumeOutput;  /*!< \brief Kind of the output printed to the history file. */
  ...
  /* For Agony Test */
  bool useComputeUnderRelaxationFactor; 
  su2double quantum_error_threshold;
  su2double linf_sampling_constant;
  bool use_quantum;
  unsigned short linf_tomography_version;

  bool use_my_linear_solver;
  bool check_solver_result;
  su2double check_solver_result_error;

  bool output_histogram;
  unsigned long histogram_space;
  unsigned long output_histogram_freq;
...
public:
  ...
  */
   ...
   */
  unsigned long GetWrt_Sol_Freq(void) const { 
      return Wrt_Sol_Freq; 
  }
  ...
  */
  ...
  /* For Agony Test */
  bool GetUseComputeUnderRelaxationFactor() const {
      return useComputeUnderRelaxationFactor;
  }

  su2double GetQuantumErrorThreshold() const {
      return quantum_error_threshold;
  }

  su2double GetLinfSamplingConstant() const {
      return linf_sampling_constant;
  }

  bool GetUseQuantum() const {
      return use_quantum;
  }

  unsigned short GetLinfTomographyVersion() const {
      return linf_tomography_version;
  }

  bool GetUseMyLinearSolver() const { return use_my_linear_solver; }
  bool GetCheckSolverResult() const { return check_solver_result; }
  bool GetCheckSolverResultError() const { return check_solver_result_error; }

  bool GetOutputHistogram() const { return output_histogram; }
  unsigned long GetHistogramSpace() const { return histogram_space; }
  unsigned long GetOutputHistogramFreq() const { return output_histogram_freq; }
